...
I guess {this tweet}[http://twitter.com/#!/projectzebra/status/27473779294736384] was what
...
go.
    ...
    gem install sinatra json hpricot

== Install

There is no gem. Clone it and go:
...
cURL examples are always best for this kind of thing, I find.
   ...
   curl -X POST -d '{"text":"Where can I get the bus?"}' http://localhost:4567/scottish
...
There's a good chance that I have a live version of this up on Heroku, just
because it is so trivially easy to deploy there. If the following works, then
it's running.

   curl -X POST -d '{"text":"Where can I get the bus?"}' http://sleekit.heroku.com/scottish
